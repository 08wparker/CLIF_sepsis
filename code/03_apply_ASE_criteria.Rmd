...
qualifying_antibiotic_days <- qualifying_antibiotic_days %>%
  ...
  summarise(total_QAD = n()) %>%# Count qualifying days per blood culture collection
  filter(total_QAD >=4)

# Step 4: Load death times and join with essential fields
death_times <- open_dataset(file.path(tables_path, "clif_hospitalization_cohort.parquet")) %>%
...
presumed_infection <- qualifying_antibiotic_days %>%
  ...
  mutate(
    presumed_infection = ifelse(
      total_QAD >= 4 | (total_QAD >= 1 & !is.na(death_dttm) & death_dttm < (blood_culture_collect_time + days(4))), 
      1, 0
    )
  ) %>%
