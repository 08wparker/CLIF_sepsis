...
#Ask Will Parker about whether this needs to be changed, given our internal issue at UoC to identify blood cultures
```{r}
...
#Pull presumed infection ids
```{r}
presumed_infected_ids <- presumed_infection %>% pull(hospitalization_id) %>% unique()
```

#Calculate Time_to_presumed_infection = difference in hours between admission and blood_culture collect time (time)
```{r}
presumed_infection %>%
  ...
  lims(x =c(0,24*7)) + 
  labs(x = "Hours since admission", title = "Time to blood culture collection from admission for patients with presumed infection")
```
...
#time is vasoactive administration
#presumed infection time is blood culture collection time

```

```{r}
#pull hospitalization ids for patients that qualify for vasopressors 
...
```{r}
#pull hospitalization ids for patients that qualify for IMV
...
wide_sepsis_monitoring_dataset <-long_sepsis_labs %>%
  ...
  summarize(lab_value_numeric = mean(lab_value_numeric, na.rm = TRUE)) %>% #this is the mean value 
...
wide_sepsis_times <- wide_sepsis_times %>%
  ...
  mutate(earliest_time_no_lactate = pmin(`invasive mechanical ventilation`, vasopressor, aki, 
         ...
         earliest_time_with_lactate = pmin(`invasive mechanical ventilation`, vasopressor, aki, hyperbilirubinemia, thrombocytopenia, lactate, na.rm = TRUE)) %>%
  ...
  mutate(dx_time_gained_with_lactate = as.numeric(difftime(earliest_time_no_lactate, earliest_time_with_lactate, units = "hours"))) %>%
...
#Step 6 create column that list the first organ failure, with and without inclusion of lactate 
  # Create 'first_organ_dysfunction_with_lactate' by finding which column has the minimum value across the specified columns
  mutate(
    first_organ_dysfunction_with_lactate = case_when(
      thrombocytopenia == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, lactate, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "thrombocytopenia",
      aki == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, lactate, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "aki",
      `invasive mechanical ventilation` == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, lactate, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "invasive mechanical ventilation",
      lactate == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, lactate, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "lactate",
      vasopressor == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, lactate, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "vasopressor",
      hyperbilirubinemia == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, lactate, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "hyperbilirubinemia",
      TRUE ~ NA_character_
    ),
    
    # Create 'first_organ_dysfunction_without_lactate' excluding 'lactate'
    first_organ_dysfunction_without_lactate = case_when(
      thrombocytopenia == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "thrombocytopenia",
      aki == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "aki",
      `invasive mechanical ventilation` == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "invasive mechanical ventilation",
      vasopressor == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "vasopressor",
      hyperbilirubinemia == pmin(thrombocytopenia, aki, `invasive mechanical ventilation`, vasopressor, hyperbilirubinemia, na.rm = TRUE) ~ "hyperbilirubinemia",
      TRUE ~ NA_character_
    )
  ) %>% 
  select(hospitalization_id, earliest_time_no_lactate, first_organ_dysfunction_without_lactate, earliest_time_with_lactate, first_organ_dysfunction_with_lactate, dx_time_gained_with_lactate, lactate_early_dx, lactate_only_dx, everything()) 
