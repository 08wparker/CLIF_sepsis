...
sepsis_incidence <- tibble(
  site = as.factor("ucmc"),  # Set 'site' as "ucmc" for all rows
  sepsis_n_including_lactate,  # Example count, replace with your actual value
  sepsis_n_becauseof_lactate,  # Example count, replace with your actual value
  sepsis_n_excluding_lactate,
  total_hospitalizations) %>%  # Example count, replace with your actual value
...
glue(
"The sepsis incidence was {round(100 * sepsis_n_including_lactate / total_hospitalizations, digits = 2)}% for adult patients admitted during 2020-2021. 
The sepsis incidence without lactate was {round(100 * sepsis_n_excluding_lactate / total_hospitalizations, digits = 2)}% for adult patients admitted during 2020-2021."
)

```
##If reasonable sepsis incidence, please export your table into box
```{r}
#sepsis_incidence %>% write.csv(***pathway to dropbox***)
sepsis_incidence
```
...
```{r}
#all patients who meet part B sepsis criteria (lactate, vasopressos, IMV, creatinine, bili, platelets) including use of lactate 
sepsis_cases_time_withlactate_time <- wide_sepsis_times %>%
...
#patients who meet part B sepsis criteria because of addition of lactate 
sepsis_n_becauseof_lactate_time <- wide_sepsis_times %>%
  filter(lactate_only_dx == 1) %>%
  select(hospitalization_id, earliest_time_with_lactate) %>%
  mutate(year = year(earliest_time_with_lactate),
         month = month(earliest_time_with_lactate)) %>%
...
sepsis_incidence_time <- sepsis_n_excluding_lactate_time %>% 
  ...
  mutate(site = as.factor("ucmc")) #please insert your site name here 
...
```
#If the above graph looks correct, please export your table into box
```{r}
#sepsis_incidence_time %>% write.csv(***pathway to dropbox***)
sepsis_incidence_time
```

#assessment of lactate ordering trends over time; number of lactate ordered per sepsis patient hospitalization
```{r}
lactate_order_per_patient_full_hospitalization <- wide_sepsis_times %>%
...
lactate_order_per_patient_until_sepsis <- wide_sepsis_times %>%
  left_join(clif_labs_cohort %>%
              select(hospitalization_id, lab_result_dttm, lab_category),
...
# If the above graph looks correct, please export your table into box
```{r}
#lactate_order_per_patient %>%  write.csv(***pathway to dropbox***))
lactate_order_per_patient
```

#number lactate orders over time
```{r}
lactate_order_per_patient_full_hospitalization_time <- wide_sepsis_times %>%
  left_join(clif_labs_cohort %>%
              select(hospitalization_id, lab_result_dttm, lab_category),
            by = "hospitalization_id") %>%  # Join on hospitalization_id
  filter(lab_category == "lactate") %>% 
...
lactate_order_per_patient_until_sepsis_time <- wide_sepsis_times %>%
  left_join(clif_labs_cohort %>%
              select(hospitalization_id, lab_result_dttm, lab_category),
            by = "hospitalization_id")  %>% 
  filter(lab_category == "lactate") %>% 
...
# If the above graph looks correct, please export your table into box
```{r}
#lactate_order_per_patient_time %>%  write.csv(***pathway to dropbox***))
```

#calculate the proportions of first organ dysfunctions, with and without lactate criteria
...
without_lactate <- wide_sepsis_times %>%
  ungroup() %>%
...
with_lactate <- wide_sepsis_times %>%
  ungroup() %>%
...
```{r}
#export table
#first_organ_dysfunction_wi_wo_lactacte %>% write.csv(***pathway to dropbox***)
first_organ_dysfunction_wi_wo_lactacte
```
...
time_gained_with_lactate <- wide_sepsis_times %>%
  ...
  mutate(site = as.factor("ucmc"))  # Add your site as 'ucmc'
...
time_gained_with_lactate_filtered %>% 
 ...
 labs(title = "Time Gained for Part B Organ Dysfunction Diagnosis from Lactate Inclusion",
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
```
# #If the above dsitribution looks correct (majority of time gained should be few hours max), please export your table into box
...
```{r}
#export table
#time_gained_with_lactate %>% write.csv(***pathway to dropbox***)
```
...
wide_sepsis_times_organfailures <- wide_sepsis_times %>%
...
table1_wide_lactate_excuded<- wide_sepsis_times_organfailures %>% 
...
table1_wide_lactate_included <- wide_sepsis_times_organfailures %>% 
...
table1_wide_lactate_only <- wide_sepsis_times_organfailures %>% 
...
```{r}
# table1_inclusion_exclusion_lactate_partb %>%  write.csv(***pathway to dropbox***)
```
