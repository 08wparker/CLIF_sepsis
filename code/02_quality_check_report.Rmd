...
```{r}
clif_medication_admin_intermittent_cohort %>% 
  rename(med_dose_unit = med_dose_units) 

clif_medication_admin_intermittent_cohort %>%
  select(med_category, med_dose, med_dose_unit) %>%
  mutate(med_dose = as.numeric(med_dose)) %>%
  mutate(full_med = paste(med_dose,med_dose_unit)) %>%
...
```{r}
long_sepsis_labs %>%
  filter(tolower(lab_category) %in% c("creatinine", "bilirubin_total", "platelet_count")) %>%
